...
export default function EditFeedstockScreen() {
  ...
  const { data: feedstock, isLoading } = useQuery<FeedstockData>({
    queryKey: ['feedstock', id],
    queryFn: () => api.feedstock.get(id!) as Promise<FeedstockData>,
    enabled: !!id,
  });
  ...
  useEffect(() => {
    if (feedstock) {
      setFormData({
        ...
        weightTonnes: feedstock.weightTonnes.toString(),
        moistureContent: feedstock.moistureContent?.toString() || '',
        sourceDescription: feedstock.sourceDescription || '',
        supplierName: feedstock.supplierName || '',
      });
    }
  }, [feedstock]);

  const updateMutation = useMutation({
    mutationFn: (data: typeof formData) => api.feedstock.update(id!, {
      ...data,
    ...
    onError: () => {
      Alert.alert('Error', 'Failed to update feedstock delivery');
    },
  });
  ...
  const handleDelete = () => {
    Alert.alert(
      'Delete Feedstock',
      'Are you sure you want to delete this feedstock delivery? This action cannot be undone.',
      [
